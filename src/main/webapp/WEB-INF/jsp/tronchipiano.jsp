    ...
    <ul>
        <%
            ...
            for(Map.Entry<TroncoEntity, HashMap<String, Float>> entry : map.entrySet()) {
                out.println("<li style=\"margin-top:20px\"><strong>Id Tronco:</strong> "+entry.getKey().getId()+", BeaconA: "+entry.getKey().getBeaconEstremi().get(0).getId() +", BeaconB: "+ entry.getKey().getBeaconEstremi().get(1).getId() +", Area: " + entry.getKey().getArea() +" mq. </li>");
            ...
            out.println("<br><input value=\"Submit\" type=\"submit\" class=\"coolbutton\" style=\"margin-top:30px; margin-bottom:30px;padding: 10px 20px 10px 20px;\">");
            out.println("</form>");
        %>
    </ul>
</div>
